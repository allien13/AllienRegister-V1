...
if(!isim) return message.channel.send(new MessageEmbed().setDescription(`Bir isim girmeyi unuttunuz. \`.i @allien/ID <İsim Yaş>\` `).setTimestamp().setFooter(ayar.footer))
if(!yas) return message.channel.send(new MessageEmbed().setDescription(`Bir yaş girmeyi unuttunuz. \`.i @allien/ID <İsim Yaş>\` `).setTimestamp().setFooter(ayar.footer))
